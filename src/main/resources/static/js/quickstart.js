$(function () {
    ...
    const maintenanceMode = window.maintenanceMode || {
        enabled: false,
    ...
    function wireUiEvents() {
        ...
        let clearLongPress = false;

        clearButton.addEventListener('pointerdown', function (e) {
            e.preventDefault();
        ...
        clearButton.addEventListener('pointerup', function (e) {
            e.preventDefault();
            ...
            if (start !== end) {
                ...
                // Delete one digit before cursor
                input.value =
        ...
        clearButton.addEventListener('pointercancel', function () {
            if (clearPressTimer) {
                clearTimeout(clearPressTimer);
                clearPressTimer = null;
            }
        ...
        speakerDevices.addEventListener('change', function () {
            ...
            const ids = Array.from(speakerDevices.selectedOptions).map(function (o) {
        ...
        ringtoneDevices.addEventListener('change', function () {
            if (!device || !device.audio) return;
            const ids = Array.from(ringtoneDevices.selectedOptions).map(function (o) {
                return o.getAttribute('data-id');
            });
            ...
            if (firebaseUser) {
                firebaseUser.getIdToken(false)
                    .then(function (idToken) {
                        return handleGoogleAuthSuccess(idToken);
                    })
                    .catch(function (err) {
        ...
        $.ajax({
            ...
            .then(function (session) {
                ...
                if (token) {
                    ...
                    afterLogin = $.getJSON('/user/token').then(function (td) {
                        startLoggedInUi(session, td.token);
                    });
                }
                return afterLogin.then(function () {
                    resumePendingCall();
                });
            })
            .catch(function (error) {
        ...
        $.ajax({url: '/user/profile', method: 'POST', contentType: 'application/json', data: JSON.stringify(payload)})
            .then(function (session) {
                ...
                if (token) {
                    ...
                    return $.getJSON('/user/token').then(function (td) {
                        startLoggedInUi(session, td.token);
                    });
                }
                resumePendingCall();
        ...
        if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
            navigator.mediaDevices.getUserMedia({audio: true})
                .then(function (preWarmStream) {
                    preWarmStream.getTracks().forEach(function (t) {
                        t.stop();
                    });
    ...
    function refreshPermissionUI() {
        if (permMicBadge) {
            permMicBadge.className = 'perm-badge perm-badge-checking';
            permMicBadge.textContent = 'Checking…';
        }
        ...
        if (permModalActions) {
            ...
            if (btnRequestMic) {
                ...
                btnRequestMic.textContent = 'Allow Microphone';
            }
        }
        getMicState().then(function (state) {
            ...
            if (state === 'granted') {
                if (permMicBadge) {
                    permMicBadge.className = 'perm-badge perm-badge-granted';
                    permMicBadge.textContent = 'Granted ✓';
                }
                if (permMicHint) {
                    permMicHint.textContent = 'Microphone access is allowed. You can make calls.';
                    permMicHint.style.display = 'block';
                }
                if (permModalActions) permModalActions.classList.add('mic-granted');
            } else if (state === 'denied') {
                if (permMicBadge) {
                    permMicBadge.className = 'perm-badge perm-badge-denied';
                    permMicBadge.textContent = 'Blocked ✕';
                }
                if (permMicHint) {
                    permMicHint.textContent = 'Microphone access is blocked by your browser. Follow the steps below to unblock it.';
                    permMicHint.style.display = 'block';
                }
                ...
                if (permMicBadge) {
                    permMicBadge.className = 'perm-badge perm-badge-prompt';
                    permMicBadge.textContent = 'Not Granted';
                }
                if (permMicHint) {
                    permMicHint.textContent = 'Tap "Allow Microphone" below. Your browser will ask for permission — tap Allow.';
                    permMicHint.style.display = 'block';
                }
            } else {
                ...
                if (permMicBadge) {
                    permMicBadge.className = 'perm-badge perm-badge-prompt';
                    permMicBadge.textContent = 'Tap to Check';
                }
                if (permMicHint) {
    ...
    function requestMicPermission() {
        ...
        if (btnRequestMic) {
            ...
            btnRequestMic.textContent = 'Requesting…';
        }
        navigator.mediaDevices.getUserMedia({audio: true})
    ...
    function stopCallTimer() {
        ...
        if (callTimerEl) {
            callTimerEl.classList.remove('active');
    ...
    function topUpWallet() {
        ...
        if (rechargeModal) {
            if (rechargeAmountEl) {
                rechargeAmountEl.value = '50';
            }
            // Default to Razorpay (most users/test runs expect Razorpay).
            if (rechargeGatewayEl) {
                rechargeGatewayEl.value = 'razorpay';
            }
            rechargeModal.classList.add('open');
            setTimeout(function () {
                if (rechargeAmountEl) {
                    rechargeAmountEl.focus();
                    rechargeAmountEl.select();
                }
            }, 150);
        }
    }
    ...
    function openRazorpayCheckout(order) {
        ...
        const options = {
            ...
            modal: {
                ondismiss: function () {
                    log('Payment cancelled. Wallet was not credited.');
                }
            }
        };
    ...
    function applyMaintenanceModeUi() {
        ...
        if (topUpButton) {
            ...
            topUpButton.title = message;
        }
        if (phoneNumberInput) {
            phoneNumberInput.disabled = true;
            phoneNumberInput.placeholder = 'Maintenance mode active';
        }
        setDeviceStatus('Maintenance', 'error');
        log(message);
    }
    ...
    function updateDevices(selectEl, selectedDevices) {
        ...
        device.audio.availableOutputDevices.forEach(function (deviceInfo, id) {
            ...
            selectedDevices.forEach(function (sel) {
                if (sel.deviceId === id) isActive = true;
            });
    ...
    function handleDialKey(key) {
        ...
        // Put cursor after the inserted digit.
        ...
        input.classList.remove('digit-added');
        void input.offsetWidth;
        input.classList.add('digit-added');
    }
    ...
    phoneNumberInput.addEventListener('input', function () {
        ...
        // If there are more than 10 digits, keep the LAST 10.
        if (digits.length > 10) {
            const oldStart = input.selectionStart ?? value.length;
            const oldEnd = input.selectionEnd ?? oldStart;

            input.value = digits.slice(-10);

            // Best-effort cursor preservation.
            const removedFromStart = digits.length - 10;
            const newStart = Math.max(0, oldStart - removedFromStart);
            const newEnd = Math.max(0, oldEnd - removedFromStart);

            try {
                input.setSelectionRange(
                    Math.min(newStart, input.value.length),
                    Math.min(newEnd, input.value.length)
                );
            } catch (_) {
                // Some mobile browsers may not allow selection updates here.
            }

            return;
        }

        // Only rewrite when the browser supplied non-digit characters.
        // This avoids unnecessarily disturbing the cursor.
        if (value !== digits) {
            const oldStart = input.selectionStart ?? value.length;
            const oldEnd = input.selectionEnd ?? oldStart;

            const beforeStart = value.slice(0, oldStart);
            const beforeEnd = value.slice(0, oldEnd);

            input.value = digits;

            const newStart = beforeStart.replace(/\D/g, '').length;
            const newEnd = beforeEnd.replace(/\D/g, '').length;

            try {
                input.setSelectionRange(newStart, newEnd);
            } catch (_) {
                // Ignore browsers that don't support selection updates here.
            }
        }
    });
